...
from app.agents.CodeConveneel.agent import CodeConveneelAgent
...
from app.agents.CodeConveneel.nodes.summarization import store_summary_to_database
...
class AgentCoordinator:
    ...
    def __init__(self, queue_manager: AsyncQueueManager):
        ...
        self.CodeConveneel_agent = CodeConveneelAgent()
    ...
    def _register_handlers(self):
        ...
        self.queue_manager.register_handler("CodeConveneel_request", self._handle_CodeConveneel_request)
    ...
    @traceable(name="CodeConveneel_request_coordination", run_type="chain")
    async def _handle_CodeConveneel_request(self, message_data: Dict[str, Any]):
        """Handle CodeConveneel agent requests"""
        try:
            ...
            # Run agent
            logger.info(f"Running CodeConveneel agent for session {session_id} with memory thread {memory_thread_id}")
            result_state = await self.CodeConveneel_agent.run(initial_state, memory_thread_id)
            ...
            logger.error(f"Error handling CodeConveneel request: {str(e)}")
    ...
    async def _handle_clear_memory_request(self, message_data: Dict[str, Any]):
        ...
        try:
            ...
            success = await self.CodeConveneel_agent.clear_thread_memory(memory_thread_id, force_clear=True)
    ...
    async def _handle_memory_timeout(self, memory_thread_id: str, state: AgentState):
        ...
        try:
            ...
            # Clear from InMemorySaver
            await self.CodeConveneel_agent.clear_thread_memory(memory_thread_id, force_clear=True)
